...
якорь pf. Проверено на своей macOS-виртуалке (установка, TUN, tpws,
kill-switch, удаление).

**iOS** — заготовка, не собиралась (нужен Xcode с iOS SDK и аккаунт Apple
Developer): ядро в приложении, как на Android, туннель — в расширении
`NEPacketTunnelProvider` с libbox, обхода DPI нет. Подробно —
`app/ios/README.md`.
