...
def step_updater(ssh, cfg, global_cfg, enable_autocheck=True):
    ...
    # Owner/repo must never be empty — a blank GH_OWNER=/GH_REPO= in update.conf
    # overrides detour-update's defaults and breaks self-update. Fall back to the
    # canonical public repo.
    gh_owner = gh.get("owner", "") or "varyen"
    gh_repo = gh.get("repo", "") or "detour"
