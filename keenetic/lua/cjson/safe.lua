-- cjson.safe — pure-Lua drop-in for lua-cjson's "safe" API (encode/decode return
-- nil + error message instead of raising). Bundled for Entware/Keenetic, where the
-- C `lua-cjson` package is NOT in the feed. Shipped at /opt/share/lua/5.1/cjson/safe.lua
-- so `require("cjson.safe")` resolves to it.
--
-- JSON core adapted from rxi/json.lua (MIT). The subscription-refresh script only
-- needs encode/decode (no cjson.null / empty-array sentinels), so this is sufficient.
...
local function encode_table(val, stack)
  ...
  if next(val) == nil then
    stack[val] = nil
    return "{}"                       -- empty table -> object (cjson default)
  elseif rawget(val, 1) ~= nil then
    -- array: keys must be 1..n, dense
    local n = 0
    for k in pairs(val) do
      if type(k) ~= "number" then error("invalid table: mixed key types") end
      n = n + 1
    end
    ...
    for k, v in pairs(val) do
      if type(k) ~= "string" then error("invalid table: object key must be string") end
...
function safe.encode(v)
  local ok, res = pcall(json.encode, v)
  if ok then return res end
  return nil, res
end

function safe.decode(s)
  local ok, res = pcall(json.decode, s)
  if ok then return res end
  return nil, res
end

safe.null = nil   -- this impl maps JSON null -> Lua nil (script doesn't use a sentinel)
