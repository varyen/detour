...
export interface WanUplink {
  ...
  up?: boolean;
  /** Адрес есть, но балансировщик пометил маршрут `dead` — канал не проходит.
      Без этого признака `up && !active` читается как «исправный резерв», хотя
      резерва нет: адрес от провайдера ещё не значит транзит в интернет. */
  route_dead?: boolean;
  proto?: string;
  ip?: string;
  gateway?: string;
