...
**Версия:** [`1.43.0`](VERSION) · **История изменений:** [`CHANGELOG.md`](CHANGELOG.md)
...
- **Проброс сервисов** (`detour-portmap`) — публикация домашнего устройства
  наружу из панели: либо HTTPS-реверс-прокси на сертификате панели
  (`https://<домен>:<порт>/` → `http://<ip>:<порт>` в LAN, с WebSocket и
  опциональной Basic-авторизацией), либо обычный проброс порта (DNAT) для
  любого протокола.
