...
**Версия:** [`1.55.0`](VERSION) · **История изменений:** [`CHANGELOG.md`](CHANGELOG.md)
...
- **Cloudflare WARP** (`detour-warp`) — бесплатный WireGuard-туннель Cloudflare
  последним хопом цепочки (клиент → ваш VPN → WARP → сайт). Выходной адрес
  перестаёт выглядеть как «сервер в дата-центре» — это то, что обычно нужно,
  чтобы заработали ИИ-сервисы, отказывающие хостингам. Регистрация — в диалоге
  «⚙ → Регистрация WARP»: выбираешь VPN-окружение, роутер регистрирует устройство
  через него (не трогая активный трафик) и сразу сохраняет готовую цепочку
  «VPN → WARP». Можно завести несколько цепочек под разные выходы.
- **Именованные цепочки** — сохранённый порядок профилей (VPN1 → VPN2 → …). Их
  видно на главном экране (включаются одним кликом) и можно назначить целью в
  «Карте маршрутов» — например увести только `gemini.google.com` в
  «Германия → WARP», оставив остальной трафик на другом VPN.
- **Проброс сервисов** (`detour-portmap`) — публикация домашнего устройства
  наружу из панели, три режима: **по имени** (`https://hass.h.example.com/` —
  все сервисы делят 443-й порт, различаются по SNI/Host; нужен покрывающий имя
  сертификат), **по порту** (`https://<домен>:<порт>/`) и обычный **проброс
  порта** (DNAT) для любого протокола. Первые два — реверс-прокси на
  сертификате панели, с WebSocket и опциональной Basic-авторизацией. Одно
  приложение можно опубликовать сразу под несколькими именами, а передачу
  `X-Forwarded-For` можно выключить для сервисов, которые отвечают на него
  `400` (Home Assistant и подобные).
- **HTTPS-сертификат** (`detour-cert`) — Let's Encrypt через acme.sh двумя
  способами: HTTP-01 (нужен доступный снаружи 80-й порт) и **DNS-01 через API
